...
import { 
  ...
  ToggleButton, ToggleButtonGroup, InputAdornment, IconButton, Grid, Paper, Typography, Chip, Divider, Box
} from '@mui/material';
import { 
  ...
  Mic as MicIcon // Icona per il Simulatore
} from '@mui/icons-material';
import toast from 'react-hot-toast';
import AICoachModal from '../components/AICoachModal'; 
import InterviewModal from '../components/InterviewModal'; // <--- IMPORTA IL NUOVO COMPONENTE
...
export default function Jobs() {
  ...
  // Stati AI - Interview Simulator (STEP 4)
  ...
  // --- NUOVA FUNZIONE: INTERVIEW SIMULATOR ---
    ...
    try {
      const res = await fetch('/api/ai/generate', {
        ...
        body: JSON.stringify({ 
            ...
            tone: letterTone, // 'formal', 'modern', 'bold'
      ...
      if (res.ok) {
        ...
        // --- 🚨 FIX PER NUOVO FORMATO BACKEND ---
        let finalLetter = "";

        if (data.variants && Array.isArray(data.variants)) {
            // Se il backend ci manda le 3 varianti, cerchiamo quella che corrisponde al tono selezionato
            // Mappiamo i toni del frontend (formal, modern, bold) con quelli del prompt (Formale, Neutra/Modern, Confidente)
            const toneMap = {
                'formal': 'Formale',
                'modern': 'Neutra', // O "Neutro"
                'bold': 'Confidente'
            };
            
            const targetType = toneMap[letterTone] || 'Formale';
            
            // Trova la variante giusta o usa la prima disponibile
            const variant = data.variants.find(v => v.type && v.type.includes(targetType)) || data.variants[0];
            finalLetter = variant.letter;
        } else {
            // Fallback per retrocompatibilità (se il backend manda ancora il vecchio formato)
  ...
  return (
    <div className="max-w-[1920px] mx-auto px-4 sm:px-6 py-6 min-h-screen text-slate-900 dark:text-slate-100 pb-24 transition-colors duration-300">
      ...
      {/* MODALE INTERVIEW SIMULATOR (NUOVO!) */}
      <InterviewModal 
        open={interviewOpen} 
