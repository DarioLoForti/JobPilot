...
import { 
  Visibility, VisibilityOff, CheckCircle, Cancel, Google 
...
export default function Register() {
  ...
  const [validity, setValidity] = useState({ length: false, upper: false, lower: false, number: false, special: false });
  ...
  useEffect(() => {
    if (formData.email.length > 0) setEmailValid(rules.email(formData.email));
    else setEmailValid(true);
  }, [formData.email]);

  const isFormValid = Object.values(validity).every(Boolean) && emailValid && formData.email.length > 0 && formData.first_name && formData.last_name;
  ...
  const textFieldStyle = {
    '& .MuiOutlinedInput-root': { '& fieldset': { borderColor: 'rgba(148, 163, 184, 0.4)' }, '&:hover fieldset': { borderColor: '#6366f1' } },
  ...
  const PasswordRequirement = ({ met, label }) => (
    <ListItem dense sx={{ py: 0, minHeight: 24 }}>
      <ListItemIcon sx={{ minWidth: 28 }}>{met ? <CheckCircle sx={{ fontSize: 16, color: '#10b981' }} /> : <Cancel sx={{ fontSize: 16, color: '#94a3b8' }} />}</ListItemIcon>
    ...
    <div className="flex items-center justify-center min-h-screen bg-slate-100 dark:bg-slate-900 px-4 transition-colors duration-300 py-10">
      <Paper className="p-8 md:p-10 w-full max-w-lg rounded-3xl shadow-xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 animate-fade-in">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <TextField fullWidth label="Email" name="email" type="email" variant="outlined" value={formData.email} onChange={handleChange} required error={!emailValid && formData.email.length > 0} helperText={!emailValid && formData.email.length > 0 ? "Inserisci un indirizzo email valido" : ""} className="bg-slate-50 dark:bg-slate-900 rounded-lg" sx={textFieldStyle} />
          <Box>
            <TextField fullWidth label="Password" name="password" type={showPassword ? 'text' : 'password'} variant="outlined" value={formData.password} onChange={handleChange} onFocus={() => setPasswordFocus(true)} required className="bg-slate-50 dark:bg-slate-900 rounded-lg" InputProps={{ endAdornment: (<InputAdornment position="end"><IconButton onClick={() => setShowPassword(!showPassword)} edge="end" className="text-slate-500 dark:text-slate-400">{showPassword ? <VisibilityOff /> : <Visibility />}</IconButton></InputAdornment>) }} sx={textFieldStyle} />
